...
class _Range{
	...
	_wrap(func){
		...
		return new _Range(this.start, this.end, this.step, new_func)
	}
	...
	reverse(){
		const end = this.start + Math.ceil((this.end - this.start) / this.step) * this.step
		const new_start = end - this.step
		const new_end = this.start - this.step
		const new_step = -this.step
		return new _Range(new_start, new_end, new_step, this._func)
	}

	reduceRight(reducer, initial_value = 0){
		return this.reverse().reduce(reducer, initial_value)
	}
	...
	// cond_func(x)
	count(cond_func){
		return this.reduce((acc, val) => !!cond_func(val) ? acc + 1 : acc, 0)
	}
